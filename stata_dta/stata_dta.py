...
class Dta():
    ...
    def __init__(self, *args, **kwargs):
        ...
        >>> Dta118("path/to/some_dta_v118.dta")
    ...
    def _new_from_file(self, address, quiet=False):
        ...
        if version in (114, 115):
            ...
            self._file_to_Dta118(address)
            if not isinstance(self, Dta118):
                if not quiet:
                    msg = "file format is {}, converting to 117 or 115"
                    print(msg.format(version))
                self._convert_dta(Dta118)
    ...
    def _file_to_Dta118(self, address):
        """populate fields of dta object with values from disk"""
        missing_above = {65530: 100, 65529: 32740, 65528: 2147483620,
                         65527: float.fromhex('0x1.fffffep+126'),
                         65526: float.fromhex('0x1.fffffffffffffp+1022')}
        ...
        with open(address, 'rb') as sfile:
            ...
            if get_str(9) != "<release>":
                raise DtaParseError("expected '<release>'")
            self._ds_format = int(get_str(3))
            print("文档版本号：", self._ds_format)
            if self._ds_format != 118:
                raise DtaParseError("expected release 118")
            if get_str(10) != "</release>":
                raise DtaParseError("expected '</release>'")
            ...
            if get_str(11) != "<byteorder>":
                raise DtaParseError("expected '<byteorder>'")
            self._byteorder = byteorder = ('>' if get_str(3) == "MSF" else '<')
            byteorder_str = "大端" if byteorder == ">" else "小端"
            print("数据大小端格式：", byteorder_str)
            if get_str(12) != "</byteorder>":
                raise DtaParseError("expected '</byteorder>'")
            ...
            if get_str(3) != "<K>":
                raise DtaParseError("expected '<K>'")
            self._nvar = nvar = unpack(byteorder + 'H', sfile.read(2))[0]
            print("变量个数: ", self._nvar)
            if get_str(4) != "</K>":
                raise DtaParseError("expected '</K>'")

            if get_str(3) != "<N>":
                raise DtaParseError("expected '<N>'")
            self._nobs = nobs = unpack(byteorder + 'Q', sfile.read(8))[0]
            print("数据行数: ", self._nobs)
            if get_str(4) != "</N>":
                raise DtaParseError("expected '</N>'")

            if get_str(7) != "<label>":
                raise DtaParseError("expected '<label>'")
            label_length = unpack(byteorder + 'H', sfile.read(2))[0]
            self._data_label = get_str(label_length)
            print("数据标签: ", self._data_label)
            if get_str(8) != "</label>":
                raise DtaParseError("expected '</label>'")
            ...
            if get_str(5) != "<map>":
                raise DtaParseError("expected '<map>'")
            self._locs = locs = unpack(byteorder + 'Q' * 14, sfile.read(14 * 8))
            print("各块信息的起始文件指针位置:", locs)
            if get_str(6) != "</map>":
                raise DtaParseError("expected '</map>'")
            ...
            self._typlist = [unpack(byteorder + 'H', sfile.read(2))[0]
                             for i in range(nvar)]
            print("类型列表(%s): %s" % (len(self._typlist), self._typlist))
            if get_str(17) != "</variable_types>":
                raise DtaParseError("expected '</variable_types>'")
            ...
            if sfile.tell() != locs[3] or get_str(10) != "<varnames>":
                raise DtaParseError("expected '<varnames>'")
            self._varlist = [get_term_str(129) for i in range(nvar)]
            print("变量列表(%s): %s" % (len(self._varlist), self._varlist))
            if get_str(11) != "</varnames>":
                raise DtaParseError("expected '</varnames>'")
            ...
            if sfile.tell() != locs[4] or get_str(10) != "<sortlist>":
                raise DtaParseError("expected '<sortlist>'")
            self._srtlist = self._get_srtlist(sfile)
            print("顺序列表(%s): %s" % (len(self._srtlist), self._srtlist))
            if get_str(11) != "</sortlist>":
                raise DtaParseError("expected '</sortlist>'")
            ...
            if sfile.tell() != locs[5] or get_str(9) != "<formats>":
                raise DtaParseError("expected '<formats>'")
            self._fmtlist = [get_term_str(57) for i in range(nvar)]
            print("格式列表(%s): %s" % (len(self._fmtlist), self._fmtlist))
            if get_str(10) != "</formats>":
                raise DtaParseError("expected '</formats>'")
            ...
            if sfile.tell() != locs[6] or get_str(19) != "<value_label_names>":
                raise DtaParseError("expected '<value_label_names>'")
            self._lbllist = [get_term_str(129) for i in range(nvar)]
            print("变量标签名列表(%s): %s" % (len(self._lbllist), self._lbllist))
            if get_str(20) != "</value_label_names>":
                raise DtaParseError("expected '</value_label_names>'")
            ...
            if (not (locs[7] == 0 or sfile.tell() == locs[7]) or
                        get_str(17) != "<variable_labels>"):
                raise DtaParseError("expected '<variable_labels>'")
            self._vlblist = [get_term_str(321) for i in range(nvar)]
            print("变量标签列表(%s): %s" % (len(self._vlblist), self._vlblist))
            if get_str(18) != "</variable_labels>":
                raise DtaParseError("expected '</variable_labels>'")
            ...
            while next_four == "<ch>":
                char_len = unpack(byteorder + 'I', sfile.read(4))[0]
                s = unpack(str(char_len) + 's', sfile.read(char_len))[0]
                varname = s[:129].partition(b'\0')[0].decode('iso-8859-1')
                charname = s[129:258].partition(b'\0')[0].decode('iso-8859-1')
                charstring = s[258:].partition(b'\0')[0].decode('iso-8859-1')
                if varname not in chrdict:
                    chrdict[varname] = {}
                chrdict[varname][charname] = charstring
                if get_str(5) != "</ch>":
                ...
                next_four = get_term_str(4)
            self._chrdict = chrdict
            print("个性设置(%s): %s" % (len(self._chrdict), self._chrdict))
            if next_four != "</ch" or get_str(14) != "aracteristics>":
                raise DtaParseError("expected '</characteristics>'")

            # data
            read_data_block = False  # 略过数据的读取
            self._data_loc = locs[9]
            typlist = self._typlist
            varvals = []
            if read_data_block:
                if sfile.tell() != locs[9] or get_str(6) != "<data>":
                ...
                for _ in range(nobs):
                    ...
                    varvals.append(new_row)
                if get_str(7) != "</data>":
                    raise DtaParseError("expected '</data>'")
            self._varvals = varvals
            if not read_data_block:
                sfile.seek(locs[10], os.SEEK_SET)
...
class Dta118(Dta):
    """A Python class for Stata dataset version 118. It provides methods
    for creating, opening, manipulating, and saving Stata datasets.
    ...
    def _dta_obj_to_file(self, address):
        ...
        typlist = self._typlist_to_save
        byteorder = self._byteorder
        nvar = self._nvar_to_save
        nobs = self._nobs_to_save
        ...
        with open(address, 'wb') as dta:
            ...
            dta.write(bytearray('<header>', 'iso-8859-1'))
            dta.write(bytearray('<release>', 'iso-8859-1'))
            dta.write(bytearray('118', 'iso-8859-1'))
            dta.write(bytearray('</release>', 'iso-8859-1'))
            dta.write(bytearray('<byteorder>', 'iso-8859-1'))
            dta.write(
                bytearray('MSF' if byteorder == '>' else 'LSF', 'iso-8859-1'))
            dta.write(bytearray('</byteorder>', 'iso-8859-1'))
            dta.write(bytearray('<K>', 'iso-8859-1'))
            dta.write(pack(byteorder + 'H', nvar))
            dta.write(bytearray('</K>', 'iso-8859-1'))
            dta.write(bytearray('<N>', 'iso-8859-1'))
            dta.write(pack(byteorder + 'Q', self._nobs_to_save))
            dta.write(bytearray('</N>', 'iso-8859-1'))
            dta.write(bytearray('<label>', 'iso-8859-1'))
            label = self._data_label
            label_length = len(label)
            dta.write(pack(byteorder + 'H', label_length))
            dta.write(bytearray(label, 'iso-8859-1'))
            dta.write(bytearray('</label>', 'iso-8859-1'))
            dta.write(bytearray('<timestamp>', 'iso-8859-1'))
            stamp = self._time_stamp
            ...
            offset_map.append(dta.tell())
            dta.write(bytearray("<varnames>", 'iso-8859-1'))
            for name in self._varlist_to_save:
                name = name[:128]
                dta.write(bytearray(name, 'iso-8859-1') + b'\0' * (129 - len(name)))
            dta.write(bytearray("</varnames>", 'iso-8859-1'))

            # sort order
            offset_map.append(dta.tell())
            dta.write(bytearray("<sortlist>", 'iso-8859-1'))
            srtlist = self._srtlist_to_save + [None]
            srtlist = [srt + 1 if srt is not None else 0 for srt in srtlist]
            dta.write(pack(byteorder + 'H' * (nvar + 1), *srtlist))
            dta.write(bytearray("</sortlist>", 'iso-8859-1'))
            ...
            offset_map.append(dta.tell())
            dta.write(bytearray("<formats>", 'iso-8859-1'))
            for fmt in self._fmtlist_to_save:
                fmt = fmt[:56]
                dta.write(bytearray(fmt, 'iso-8859-1') + b'\0' * (57 - len(fmt)))
            dta.write(bytearray("</formats>", 'iso-8859-1'))

            # value-label names
            offset_map.append(dta.tell())
            dta.write(bytearray("<value_label_names>", 'iso-8859-1'))
            for lab in self._lbllist_to_save:
                lab = lab[:128]
                dta.write(bytearray(lab, 'iso-8859-1') + b'\0' * (129 - len(lab)))
            dta.write(bytearray("</value_label_names>", 'iso-8859-1'))

            # variable labels
            offset_map.append(dta.tell())
            dta.write(bytearray("<variable_labels>", 'iso-8859-1'))
            for lab in self._vlblist_to_save:
                lab = lab[:320]
                dta.write(bytearray(lab, 'iso-8859-1') + b'\0' * (321 - len(lab)))
            dta.write(bytearray("</variable_labels>", 'iso-8859-1'))

            # characteristics
            offset_map.append(dta.tell())
            dta.write(bytearray("<characteristics>", 'iso-8859-1'))
            chrdict = self._chrdict_to_save
            for varname in chrdict:
                varname = varname[:128]
                var_dict = chrdict[varname]
                for charname in var_dict:
                    charname = charname[:128]
                    char = var_dict[charname][:67784]  # or 8681 for Small Stata
                    full_length = 258 + len(char) + 1  # +1 for null termination
    ...
    def get_year_data_as_list(self, address, start_year=1998, end_year=-1, vars_to_get_list=[]):
        if end_year == -1:
            end_year = start_year
        if end_year < start_year:
            print("error: 结束年份不应该大于开始年份")
        if len(vars_to_get_list) <= 0:
            vars_to_get_list = self._varlist
        # data
        missing_above = {65530: 100, 65529: 32740, 65528: 2147483620,
                         65527: float.fromhex('0x1.fffffep+126'),
                         65526: float.fromhex('0x1.fffffffffffffp+1022')}
        ...
        year_index = self._varlist.index("year")
        file_ptr_map = {1998: 102672, 1999: 225817611, 2000: 447315355, 2001: 669975049, 2002: 901040426,
                        2003: 1149314966, 2004: 1409189868, 2005: 1790711366, 2006: 2162312545, 2007: 2910847570,
                        2008: 4048031631, 2009: 5160546810, 2010: 5988776568, 2011: 7099865966, 2012: 8070859736,
                        2013: 8709833812}
        nobs_map = {1998: 0, 1999: 165117, 2000: 327149, 2001: 490031, 2002: 659062, 2003: 840682, 2004: 1030788,
                    2005: 1309882, 2006: 1581719, 2007: 2129294, 2008: 2961177, 2009: 3775014, 2010: 4380888,
                    2011: 5193682, 2012: 5903992, 2013: 6371420, 2014: 6725518}
        vars_index_list = [self._varlist.index(var_name) for var_name in vars_to_get_list]
        vars_index_list.sort()
        print("需要获取的变量(%s):" % len(vars_index_list))
        print(vars_to_get_list)
        print(vars_index_list)
        self._nvar_to_save = len(vars_index_list)
        self._nobs_to_save = nobs_map[min(2014, end_year+1)] - nobs_map[start_year]
        self._varlist_to_save = [self._varlist[i] for i in vars_index_list]
        self._typlist_to_save = [self._typlist[i] for i in vars_index_list]
        self._varlist_to_save = [self._varlist[i] for i in vars_index_list]
        self._srtlist_to_save = [self._srtlist[i] for i in vars_index_list]
        self._fmtlist_to_save = [self._fmtlist[i] for i in vars_index_list]
        self._lbllist_to_save = [self._lbllist[i] for i in vars_index_list]
        self._vlblist_to_save = [self._vlblist[i] for i in vars_index_list]
        self._chrdict_to_save = {}
        for var_name, var_dict in self._chrdict.items():
            if self._varlist.index(var_name) in vars_index_list:
                self._chrdict_to_save[var_name] = var_dict
        ...
        last_year = 0
        with open(address, 'rb') as sfile:
            # sfile.seek(self._locs[9], os.SEEK_SET)
            # if sfile.tell() != self._locs[9] or get_str(6) != "<data>":
            #     raise DtaParseError("expected '<data>'")
            sfile.seek(file_ptr_map[start_year], os.SEEK_SET)
            for i in range(nobs_map[start_year], self._nobs):
                file_ptr = sfile.tell()
                new_row = []
                for st_type in typlist:
                    if st_type <= 2045:  # str
                        ...
                        if new_val > missing_above[st_type]:
                            new_val = missing_object(new_val, st_type)
                    new_row.append(new_val)
                if new_row[year_index] != last_year:
                    last_year = new_row[year_index]
                    print("reading year:", new_row[year_index], " file_ptr:", file_ptr, " _nobs:", i)
                if new_row[year_index] > end_year:
                    print("reading year:", new_row[year_index], " and break")
                    break
                if new_row[year_index] >= start_year:
                    varvals.append([new_row[i] for i in vars_index_list])
            # if get_str(7) != "</data>":
            #     print("Maybe errors in analizing data, expected '</data>'")
            self._varvals = varvals
        return varvals
...
def open_dta(address):
    """Open any recent version dta file (versions 114, 115, 117, 118) .
    ...
    if ds_format == 114 or ds_format == 115:
        ...
        if ds_format_118.decode('iso-8859-1') == '118':
            return Dta118(address)
        return Dta117(address)
    else:
        raise ValueError("only dta formats 118, 117, 115, and 114 are supported")
